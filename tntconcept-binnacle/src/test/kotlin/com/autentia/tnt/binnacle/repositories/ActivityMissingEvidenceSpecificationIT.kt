...
class ActivityMissingEvidenceSpecificationIT {
    ...
    private companion object {
        ...
        private val defaultApprovalState = ApprovalState.NA
    ...
    fun `should find activities without evidence and required evidence once`() {
        ...
        assertThat(result).noneMatch { this.hasEvidenceOrHookEvidence(it) }
    ...
    fun `should not find activities that have evidence with required once`() {
        val activityWithEvidence = Activity(
                ...
                userId = test_user_2,
                billable = false,
                evidences = createAttachmentInfo(),
                approvalState = defaultApprovalState
        )
    ...
    fun `should find activities missing evidence with required evidence weekly`() {
        ...
        assertThat(results).noneMatch { this.hasEvidenceOrHookEvidence(it) }
    ...
    @Test
    fun `should return missing evidence activities once excluding accepted ones`() {
        val act1 = Activity(
                start = today.atTime(8, 0, 0).minusDays(102),
                end = today.atTime(17, 0, 0).minusDays(100),
                duration = 3,
    ...
    @Test
    fun `should return missing evidence activities once excluding accepted ones with or without evidence`() {
        val act1 = Activity(
                start = today.atTime(8, 0, 0).minusDays(9),
                end = today.atTime(17, 0, 0).minusDays(9),
                duration = 540,
        ...
        activityDao.saveAll(listOf(act1, act2, act3, act4))
        val results = activityDao.findAll(activitiesMissingEvidencePredicate)
        assertThat(results).containsExactly(act3)
        assertThat(results).noneMatch() { this.hasEvidenceOrHookEvidence(it) }
    }

    private fun hasEvidenceOrHookEvidence(it: Activity): Boolean = it.hasEvidences() || it.description.startsWith("###Autocreated evidence###")
    ...
    private fun createAttachmentInfo(): MutableList<AttachmentInfo> {
        ...
        return mutableListOf(attachmentInfoDao.save(attachmentInfoForAct3))
    }
