...
@Singleton
class ActivityDateConverter(private val activityResponseConverter: ActivitiesResponseConverter) {

    fun toActivityDateDTO(activityDate: ActivityDate) = ActivityDateDTO(activityDate.date,
    ...
    fun toListActivityDate(
        ...
        val allActivitiesBetweenDates =
            startDate.myDatesUntil(endDate).associateWith { emptyList<ActivitiesResponse>() }.toMutableMap()
    ...
    private fun toActivityDate(entry: Map.Entry<LocalDate, List<ActivitiesResponse>>): ActivityDate =
        ActivityDate(date = entry.key,
            workedMinutes = entry.value.filter { it.projectRole.isWorkingTime }
