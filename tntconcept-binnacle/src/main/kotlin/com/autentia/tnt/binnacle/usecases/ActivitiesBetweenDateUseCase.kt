...
class ActivitiesBetweenDateUseCase internal constructor(
    ...
    fun getActivities(start: Optional<LocalDate>, end: Optional<LocalDate>, approvalState: Optional<ApprovalState>): List<ActivityDateDTO> {
        val user = userService.getAuthenticatedUser()

        val activities = if(start.isPresent && end.isPresent) {
            activityService.getActivitiesBetweenDates(DateInterval(start.get(), end.get()), user.id)
        } else {
